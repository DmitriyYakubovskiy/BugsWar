...
public class PlasmaTurretAI : MonoBehaviour
{
    ...
    public LayerMask turretLayerMask; //в самой Unity3D создайте новый слой дл€ мобов по аналогии с тегами и выберите его тут. я назвал его Monster. Ќе забудьте выбрать его на префабе моба.
    ...
    private void Start()
    {
        //turretHead = transform.Find("Tower"); //находим башню в иерархии частей модели
    }
        ...
        if (curTarget != null) //если переменна€ текущей цели не пуста€
        {
            ...
            if (Mathf.Pow(attackMinimumDistance, 2) < squaredDistance && squaredDistance < Mathf.Pow(attackMaximumDistance, 2)) //если дистанци€ больше мертвой зоны и меньше дистанции поражени€ пушки
            {
                ...
                if (reloadTimer <= 0)
                {
                    ...
                    {
                        Unit mhp = curTarget.GetComponent<Unit>();
                        switch (FiringOrder) //смотрим, из какого ствола стрел€ть
                        {
                            case 1:
                                if (mhp != null) mhp.TakeDamage(attackDamage); //наносим урон цели
                                FiringOrder++; //переключаем ствол
                                ammoAmount--; //минус патрон
                                break;
                            case 2:
                                if (mhp != null) mhp.TakeDamage(attackDamage);
                                FiringOrder = 1;
                                ammoAmount--;
                                break;
                        }
    ...
    private GameObject SortTargets()
    {
        ...
        //    }
        //}
        nearestmob = GameObject.FindGameObjectWithTag("Unit");
